...
<body>
판매 리스트 | <a href="customerTotal.vnt" target="_blank">고객 별 판매 현황</a> | 상품 별 판매 현황 | 년도 별 판매 현황 | 월 별 판매 현황
...
<table border=1>
	<tr><td>고객 아이디</td><td>고객명</td><td>상품명</td>
		<td>수량</td><td>판매일</td><td>배송 상태</td></tr>
	...
	<tr><td>
		...
		<td>
			...
			<c:if test="${dto.deliveryNum == null }">배송 등록</c:if>
			<c:if test="${dto.deliveryNum != null }">배송 수정</c:if>
			</a>
		</td></tr>
	</c:forEach>
</table>
