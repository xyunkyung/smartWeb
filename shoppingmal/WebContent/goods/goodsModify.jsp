...
<input type="hidden" name="prodNum" value="${dto.prodNum }" />
	<table border=1 align="center">
		...
		<tr><th>배송비</th>
			<td><input type="number" name="prodDelFee" min="0" step="500" value="${dto.prodDelFee }" /></td></tr>
		<tr><th>추천 상품</th>
			<td><input type="radio" name="recommend" value="Y" 
				<c:if test="${dto.recommend.trim() == 'Y' }">checked</c:if> />추천
				<input type="radio" name="recommend" value="N" 
				<c:if test="${dto.recommend.trim() == 'N' }">checked</c:if> />비추천</td></tr>
		<tr><th>상품 설명</th>
